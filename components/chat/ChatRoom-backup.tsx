...
// Nouveau bouton menu "⋮"
const MessageMenuButton = styled.button<{ $isOwn: boolean }>`
  position: absolute;
  top: 8px;
  ${props => props.$isOwn 
    ? 'left: -35px;'   /* À gauche pour nos messages */
    : 'right: -35px;'  /* À droite pour les autres */
  }
  width: 28px;
  height: 28px;
  border-radius: 50%;
  ...
  @media (max-width: 768px) {
    opacity: 0.4; /* Toujours légèrement visible sur mobile */
    ${props => props.$isOwn 
      ? 'left: -30px;'
      : 'right: -30px;'
    }
...
export default function ChatRoom({ onToggleSidebar, mentionName, onMentionHandled }: ChatRoomProps) {
  ...
  const handleDoubleClick = (e: React.MouseEvent | React.TouchEvent, messageId: number) => {
    ...
    // Afficher l'animation de cœur qui monte
  ...
  const handleMenuButtonClick = (e: React.MouseEvent, message: any) => {
    e.preventDefault();
    e.stopPropagation();
                    ...
                    <div>
                      ...
                      >
                        ...
                        <MessageMenuButton
                          ...
                          $isOwn={isOwn}
                          onClick={(e) => handleMenuButtonClick(e, message)}
